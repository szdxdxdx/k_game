...
static struct k_str_map_node *find(struct k_str_map *map, struct k_hash_list *list, const char *str, size_t hash) {
...
void k_init_str_map(struct k_str_map *map, struct k_hash_list *hash_lists, size_t lists_num) {
    ...
    k_init_hash_lists(hash_lists, lists_num);

    map->hash_lists = hash_lists;
...
int k_str_map_add(struct k_str_map *map, const char *key, struct k_str_map_node *node) {
    ...
    struct k_hash_list *list = &(map->hash_lists[hash % map->lists_num]);

    struct k_hash_list_node *list_node;
    if (NULL != find(map, list, key, hash))
        return -1;
...
struct k_hash_list *k_str_map_rehash(struct k_str_map *map, struct k_hash_list *new_hash_lists, size_t new_lists_num) {
    ...
    k_init_hash_lists(new_hash_lists, new_lists_num);

    struct k_hash_list *old_hash_lists = map->hash_lists;
    size_t old_lists_num = map->lists_num;

    struct k_hash_list *old_list = old_hash_lists;
    for (; old_list < old_hash_lists + old_lists_num; old_list++) {

        struct k_hash_list_node *list_node, *next;
        for (k_hash_list_for_each_s(old_list, list_node, next)) {

            struct k_str_map_node *map_node = container_of(list_node, struct k_str_map_node, list_node);
            struct k_hash_list *new_list = &(new_hash_lists[map_node->hash % new_lists_num]);
            k_hash_list_add(new_list, &map_node->list_node);
        }
    }

    map->hash_lists = new_hash_lists;
    map->lists_num = new_lists_num;

    return old_hash_lists;
}

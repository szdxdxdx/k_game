...
static inline struct k_int_intrusive_map_node *find(struct k_hash_list *list, int key) {

    struct k_int_intrusive_map_node *map_node;
    struct k_hash_list_node *iter;
    for (k_hash_list_for_each(list, iter)) {
        map_node = container_of(iter, struct k_int_intrusive_map_node, list_node);

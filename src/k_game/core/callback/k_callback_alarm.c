...
struct k_alarm_callback {
    ...
    union {
        ...
        void (*fn_component_manager_callback)(struct k_component_manager *component_manager, int timeout_diff);
    };
...
void k__alarm_callback_manager_del_callback(struct k_callback *callback) {
    ...
    k_list_del(&alarm_callback->base.context_callback_list_node);
}

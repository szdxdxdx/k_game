...
int k__font_registry_init(void) {
    return k__asset_registry_init(&k__font_registry);
}
...
void k__font_registry_cleanup(void) {
    k__asset_registry_cleanup(&k__font_registry, k__font_registry_release);
}

void k__font_registry_add(struct k_font *font) {
    k__asset_registry_add(&k__font_registry, &font->registry_node);
}
...
int k_font_set_name(struct k_font *font, const char *font_name) {
    return k__asset_set_name(&k__font_registry, &font->registry_node, font_name);
}

struct k_font *k_font_find(const char *font_name) {
    struct k_asset_registry_node *registry_node = k__asset_registry_find(&k__font_registry, font_name);

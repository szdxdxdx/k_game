...
void k__room_init_step_end_callback_registry(struct k_room *room) {
    struct k_room_step_end_callback_registry *registry = &room->step_end_callbacks;

    k_list_init(&registry->callback_list);
    k_list_init(&registry->pending_list);
}

void k__room_cleanup_step_end_callback_registry(struct k_room *room) {
    struct k_room_step_end_callback_registry *registry = &room->step_end_callbacks;

    struct k_room_step_end_callback *callback;
    struct k_list *list;
    struct k_list_node *iter, *next;

    list = &registry->callback_list;
    for (k_list_for_each_s(list, iter, next)) {
        callback = container_of(iter, struct k_room_step_end_callback, list_node);
        k_free(callback);
    }
...
struct k_room_callback *k__room_add_step_end_callback(struct k_room *room, void (*fn_callback)(void *data), void *data) {
    ...
    struct k_room_step_end_callback *callback = k_malloc(sizeof(struct k_room_step_end_callback));
    if (NULL == callback)
        return NULL;

    callback->base.callback_type = K_ROOM_STEP_END_CALLBACK;
    callback->data = data;
...
void k__room_flush_pending_step_end_callbacks(struct k_room *room) {
    struct k_room_step_end_callback_registry *registry = &room->step_end_callbacks;

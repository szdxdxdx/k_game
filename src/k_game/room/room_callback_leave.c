...
void k__room_init_leave_callback_registry(struct k_room *room) {
    struct k_room_leave_callback_registry *registry = &room->leave_callbacks;
...
void k__room_cleanup_leave_callback_registry(struct k_room *room) {
    ...
    struct k_room_leave_callback *callback;
    struct k_list *list;
    struct k_list_node *iter, *next;

    list = &registry->callback_list;
    for (k_list_for_each_s(list, iter, next)) {
        callback = container_of(iter, struct k_room_leave_callback, list_node);
        k_free(callback);
    }
...
struct k_room_callback *k__room_add_leave_callback(struct k_room *room, void (*fn_callback)(void *data), void *data) {
    ...
    struct k_room_leave_callback *callback = k_malloc(sizeof(struct k_room_leave_callback));
    if (NULL == callback)
        return NULL;

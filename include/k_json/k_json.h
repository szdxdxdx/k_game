...
/** \brief 往 json 数组节点中的指定索引处插入一个元素。添加成功返回 0，否则返回非 0 */
...
/** \brief 往 json 数组节点尾部追加一个元素。追加成功返回 0，否则返回非 0 */
int k_json_arr_push(struct k_json *json, struct k_json *val);

/** \brief 删除 json 数组节点中指定索引处的元素 */
 ...
 *     k_json_get_by_idx(3),      // 使用 `k_json_get_by_idx()` 宏，获取数组节点的元素

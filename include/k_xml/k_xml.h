 ...
 *
 * - 若 `node` 是文本节点，则函数返回文本内容字符串。
 *
 * - 若 `node` 是元素节点，则函数遍历它的所有子节点，找出第一个文本子节点，
 *   并返回该子节点的文本内容字符串。若没有文本子节点，则返回空字符串 ""。
 *
 * - 若 `node` 是注释节点，则函数返回注释内容字符串。
 *
 * - 其余情况，则函数返回 `NULL`。
 *
 * 在 xml 中，空格、换行符等空白字符也是有效的文本内容，它们是数据的一部分。
 * 若原 xml 文档中，这些字符可能是为了表示层级缩进、提高代码可读性而引入的，
 * 那你可能需要手动去除这些字符。
 *
 * 不要释放函数返回的字符串的内存，也不要原地修改该字符串内容。
 */

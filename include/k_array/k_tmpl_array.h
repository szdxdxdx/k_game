 ...
 * - k_tmpl_array_set_fn_malloc    若定义此宏，则使用指定的内存分配函数，否则使用 `malloc()`
 * - k_tmpl_array_set_fn_free      若定义此宏，则使用指定的内存释放函数，否则使用 `free()`
 * - k_tmpl_array_elem_pass_by_val 若定义此宏，则操作单个元素（例如：插入单个元素）时传递值，否则传递指针
...
#if defined(k_tmpl_array_set_fn_malloc) && defined(k_tmpl_array_set_fn_free)
#define k__tmpl_array_mem_alloc k_tmpl_array_set_fn_malloc
#define k__tmpl_array_mem_free  k_tmpl_array_set_fn_free
#elif defined(k_tmpl_array_set_fn_malloc) || defined(k_tmpl_array_set_fn_free)
/* 必须同时指明内存分配函数和释放函数 */
...
#define k__tmpl_array_static static
#else
#define k__tmpl_array_static
#endif
 ...
 */
k__tmpl_array_static struct k__tmpl_array *k__tmpl_array_(create)(size_t init_capacity);
 ...
 */
k__tmpl_array_static void k__tmpl_array_(destroy)(struct k__tmpl_array *arr);
 ...
 */
k__tmpl_array_static struct k__tmpl_array *k__tmpl_array_(construct)(struct k__tmpl_array *arr, size_t init_capacity);
 ...
 * 若 `arr` 为 `NULL`，则函数立即返回。
 */
k__tmpl_array_static void k__tmpl_array_(destruct)(struct k__tmpl_array *arr);
 ...
 * 若成功，函数返回 0，否则返回非 0。
 */
k__tmpl_array_static int k__tmpl_array_(reserve)(struct k__tmpl_array *arr, size_t n);
 ...
 * ```
 */
k__tmpl_array_static k_tmpl_array_elem_type *k__tmpl_array_(shift_right)(struct k__tmpl_array *arr, size_t idx, size_t n);
 ...
 * 若成功，函数返回 0，否则返回非 0。
 */
k__tmpl_array_static int k__tmpl_array_(insert_all)(struct k__tmpl_array *arr, size_t idx, k_tmpl_array_elem_type *elems, size_t elems_num);

#if defined(k_tmpl_array_elem_pass_by_val)

/** \brief 在数组指定位置插入一个元素，若成功，函数返回 0，否则返回非 0 */
k__tmpl_array_static int k__tmpl_array_(insert)(struct k__tmpl_array *arr, size_t idx, k_tmpl_array_elem_type elem);

#else

/** \brief 在数组指定位置插入一个元素，若成功，函数返回 0，否则返回非 0 */
k__tmpl_array_static int k__tmpl_array_(insert)(struct k__tmpl_array *arr, size_t idx, const k_tmpl_array_elem_type *elem);

#endif

/** \brief 在数组尾部追加多个元素，若成功，函数返回 0，否则返回非 0 */
k__tmpl_array_static int k__tmpl_array_(push_back_all)(struct k__tmpl_array *arr, k_tmpl_array_elem_type *elems, size_t elems_num);

#if defined(k_tmpl_array_elem_pass_by_val)

/** \brief 在数组尾部追加一个元素，若成功，函数返回 0，否则返回非 0 */
k__tmpl_array_static int k__tmpl_array_(push_back)(struct k__tmpl_array *arr, k_tmpl_array_elem_type elem);

#else

/** \brief 在数组尾部追加一个元素，若成功，函数返回 0，否则返回非 0 */
k__tmpl_array_static int k__tmpl_array_(push_back)(struct k__tmpl_array *arr, const k_tmpl_array_elem_type *elem);
 ...
 * 调用本函数后，数组 `size` 减少 `n`。
 */
k__tmpl_array_static void k__tmpl_array_(shift_left)(struct k__tmpl_array *arr, size_t idx, size_t n);

/** \brief 删除数组中指定区间（左闭右开）内的元素 */
k__tmpl_array_static void k__tmpl_array_(remove_range)(struct k__tmpl_array *arr, size_t idx_from, size_t idx_to);

/** \brief 删除数组中指定索引处的元素 */
k__tmpl_array_static void k__tmpl_array_(remove)(struct k__tmpl_array *arr, size_t idx);

/** \brief 删除数组最末尾的元素 */
k__tmpl_array_static void k__tmpl_array_(pop_back)(struct k__tmpl_array *arr);

/** \brief 清空数组，但保持原容量，不释放存储元素所使用的内存 */
k__tmpl_array_static void k__tmpl_array_(clear)(struct k__tmpl_array *arr);

/** \brief 清空数组，并释放存储元素所用的内存 */
k__tmpl_array_static void k__tmpl_array_(free_storage)(struct k__tmpl_array *arr);
...
k__tmpl_array_static int k__tmpl_array_(reserve)(struct k__tmpl_array *arr, size_t n) {
    ...
    k_tmpl_array_elem_type *new_storage = k__tmpl_array_mem_alloc(new_capacity * sizeof(k_tmpl_array_elem_type));
    if (NULL == new_storage) {
        if (new_capacity == n) {
            return -1;
        }
...
#undef k__tmpl_array_static

#undef k_tmpl_array_struct_name
#undef k_tmpl_array_elem_type
#undef k_tmpl_array_define_struct
#undef k_tmpl_array_define_fn
#undef k_tmpl_array_set_fn_malloc
#undef k_tmpl_array_set_fn_free
#undef k_tmpl_array_elem_pass_by_val
